...
int  srch1(const char snt[],const char pat[],int pos){
    string sentence = snt;
    return sentence.find(pat, pos);
}

//Search for all occurrences
void srchAll(const char snt[],const char pat[],int index[]){
    int pos = 0,
            i = 0; 
    if (srch1(snt, pat, 0) == string::npos){
        index[0] = -1; 
    }
    else{
        do{
            pos = srch1(snt, pat, pos);
            if (pos != string::npos){
                index[i] = pos;
                pos++;
                i++;
